process MTBC_READ_QC {
    ...
    // Containers build at https://wave.seqera.io/view/builds/bd-1fc53e8010f2fd05_1
    ...
    # Run kraken on failed reads (typically the negative controls to check possible contaminants)
    ...
    echo -e "${sampleID}\t${R1_num}\t${R1_qual}\t${R2_num}\t${R2_qual}\t${MTB_PERC}"
    echo -e "${sampleID}\t${R1_num}\t${R1_qual}\t${R2_num}\t${R2_qual}\t${MTB_PERC}" > samples/${sampleID}.qc.out

    # Grep the string of values from the list
