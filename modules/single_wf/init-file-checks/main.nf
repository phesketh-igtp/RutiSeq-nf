...
*/
    ...
    script:
        ...
        def mtbseq_class = "${params.outDir}/db/mtbseq/samples/${sampleID}/Classification/${sampleID}.Strain_Classification.tab"
        def mtbseq_stats = "${params.outDir}/db/mtbseq/samples/${sampleID}/Statistics/${sampleID}.Mapping_and_Variant_Statistics.tab"
        def mtbseq_pos   = "${params.outDir}/db/mtbseq/samples/${sampleID}/Position_Tables/${sampleID}.gatk_position_table.tab"
        def mtbseq_vars  = "${params.outDir}/db/mtbseq/samples/${sampleID}/Called/${sampleID}.gatk_position_variants_cf4_cr4_fr75_ph4_outmode000.tab"
        def tbdb_out     = "${params.outDir}/db/tbprofiler/results/tbdb-${sampleID}.results.txt"
        def who_out      = "${params.outDir}/db/tbprofiler/who-only/results/who-${sampleID}.results.txt"
        def mtbseq_vcf   = "${params.outDir}/db/mtbseq/samples/${sampleID}/SNP-Profiles/${sampleID}.gatk.vcf.gz"
