...
*/
    ...
    input:
        tuple val(sampleID), 
                path(mtbc_ont), //, stageAs: "${sampleID}_R1.fastq.gz"
                path(mtbseq_class), 
                path(mtbseq_stats), 
                path(mtbseq_pos), 
                path(mtbseq_vars), 
    ...
    script:
        ...
        """
        # Make the MTBseq directory structure
            mkdir -p Bam GATK_Bam \\
                Mpileup Position_Tables \\
                Called Joint Amend \\
                Classification Groups

        # Align
            minimap2 -ax map-ont ${params.mtbc_reference} ${sampleID}_ONT.fastq.gz | samtools view -b -o Bam/${sampleID}.bam -

        # Add read groups
            samtools addreplacerg \
                -r "@RG\tID:${sampleID}\tSM:${sampleID}\tPL:ONT\tLB:lib1\tPU:unit1" \
                -o Bam/${sampleID}.rg.bam Bam/${sampleID}.bam

        # Sort & index
            samtools sort -@ 4 -o Bam/${sampleID}.bam Bam/${sampleID}.rg.bam
            samtools index Bam/${sampleID}.bam
            rm Bam/${sampleID}.sam Bam/${sampleID}.rg.bam
